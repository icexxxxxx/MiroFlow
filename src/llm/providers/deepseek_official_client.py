...
class DeepSeekOfficialClient(LLMProviderClientBase):
    def _create_client(self, config: DictConfig):
        """Create configured OpenAI-compatible DeepSeek client"""
    ...
    async def _create_message(
        ...
        :param keep_tool_result: Number of tool results to keep.
        :return: DeepSeek API response object or None (if error).
        """
        logger.debug(f" Calling LLM ({'async' if self.async_client else 'sync'})")
        # Put the system prompt in the first message since DeepSeek API supports system role
        if system_prompt:
            target_role = "system"

            # Check if there are already system messages
        ...
        # DeepSeek supports tools similar to OpenAI
    ...
    def process_llm_response(
        ...
        if llm_response.choices[0].finish_reason == "stop":
            ...
            if assistant_response_text == "":
                assistant_response_text = "LLM response is empty. This is likely due to context length limit."
            else:
                assistant_response_text = self._clean_user_content_from_response(
                    assistant_response_text
                )
            ...
            # For tool_calls, extract tool call information as text
    ...
    def extract_tool_calls_info(self, llm_response, assistant_response_text):
        ...
        # For OpenAI-compatible API, directly get tool calls from response object
    ...
    def _apply_cache_control(self, messages):
        """Apply cache control to messages if supported"""
        # DeepSeek official API may have different cache control mechanism
